...
async def startup():
        ...
        try:
            ...
            if attempt == max_attempts:
                if os.getenv("ALLOW_DEGRADED_STARTUP") == "1":
                    logger.warning("Redis unavailable after retries; starting NLU in degraded mode")
                    redis_client = None
                    break
                raise
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            if attempt == max_attempts:
                if os.getenv("ALLOW_DEGRADED_STARTUP") == "1":
                    logger.warning("Postgres unavailable after retries; starting NLU in degraded mode")
                    db_pool = None
                    break
                raise
            await asyncio.sleep(delay)
...
async def health_check():
    ...
    return {"status": "healthy" if overall else "degraded", "service": "nlu", "dependencies": {"redis": bool(ok_redis), "postgres": bool(ok_db)}}
